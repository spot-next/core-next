...
public class DefaultUserService<U extends User, G extends UserGroup> extends AbstractService
	...
	public void setCurrentUser(final U user) {
		...
		if (user != null) {
			session.setAttribute(CoreConstants.SESSION_KEY_CURRENT_USER, user.getId());
		} else {
	...
	public boolean isCurrentUserAnonymous() {
		return StringUtils.isBlank(getCurrentUserId());
	}
	...
	protected String getCurrentUserId() {
		final Session session = sessionService.getCurrentSession();
		return session.getAttribute(CoreConstants.SESSION_KEY_CURRENT_USER, String.class);
	}
}