...
public final class GraphAdapterBuilder {
	...
	static class Factory implements TypeAdapterFactory, InstanceCreator {
		...
		public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
			...
			return new TypeAdapter<T>() {
				...
				public void write(JsonWriter out, T value) throws IOException {
					...
					/*
					 * We have one of two cases: 1. We've encountered the first known object in this
					 * graph. Write out the graph, starting with that object. 2. We've encountered
					 * another graph object in the course of #1. Just write out this object's name.
					 * We'll circle back to writing out the object's value as a part of #1.
					 */
				...
				public T read(JsonReader in) throws IOException {
					...
					/*
					 * Again we have one of two cases: 1. We've encountered the first known object
					 * in this graph. Read the entire graph in as a map from names to their
					 * JsonElements. Then convert the first JsonElement to its Java object. 2. We've
					 * encountered another graph object in the course of #1. Read in its name, then
					 * deserialize its value from the JsonElement in our map. We need to do this
					 * lazily because we don't know which TypeAdapter to use until a value is
		...
		/**
		 * Hook for the graph adapter to get a reference to a deserialized value before
		 * that value is fully populated. This is useful to deserialize values that
		 * directly or indirectly reference themselves: we can hand out an instance
		 * before read() returns.
		 *
		 * <p>
		 * Gson should only ever call this method when we're expecting it to; that is
		 * only when we've called back into Gson to deserialize a tree.
		 */
	...
	static class Graph {
		/**
		 * The graph elements. On serialization keys are objects (using an identity hash
		 * map) and on deserialization keys are the string names (using a standard hash
		 * map).
		 */
		...
		/**
		 * The instance currently being deserialized. Used as a backdoor between the
		 * graph traversal (which needs to know instances) and instance creators which
		 * create them.
		 */
	...
	static class Element<T> {
		...
		/**
		 * This element's type adapter if known. During deserialization this is lazily
		 * populated.
		 */
